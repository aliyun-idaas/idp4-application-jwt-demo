...
public class SystemController {
    ...
    public String loginConfigForm(@ModelAttribute("formDto") LoginPageConfigDto pageConfigDto, Model model, HttpServletRequest request) {
        loginPageConfigService.saveLoginPageConfig(pageConfigDto);
        if (StringUtils.isNotBlank(pageConfigDto.getSpLogoutUrl())) {
            request.getSession().removeAttribute("logoutUrl");
            request.getSession().setAttribute("logoutUrl", pageConfigDto.getSpLogoutUrl());
        } else {
            request.getSession().removeAttribute("logoutUrl");
        }
